...
{ 
    if(item != "" && sortname != "") 
    { 
        window.top.frames['mainFrame'].getObject('show_text').innerHTML = sortname + "  <img src=../images/mini/slide.gif broder=0 />  " + item 
    } 
    if(src != "") 
    { 
        if(src.indexOf("AdminLogin.aspx")>-1)
        {
...
function initModuleList(adminId){
    invokeAjaxModule(adminId);
}

function invokeAjaxModule(adminId) {
    //var jsonParams = { '2011-6-2', lofinid, pageIndex,  pageSize} };
    var parameterArray = new Array(adminId);
    var dataParameters = { MethodAlias: "GetAdminModule", Parameter: parameterArray };

    Ajax.Request({ url: "AjaxHandler.aspx", type: "POST", data: "data=" + escape(JSON2.stringify(dataParameters)), onComplete: renderResultModule, onError: showError });
}


function renderResultModule(xmlhttp) {
    ...
    if (result != null && result != "") {
        ...
        if (result.State == 0) {
            ...
            if (result.Data != null && result.Data != "") {
                var mainInfo = result.Data.toJSON();
                t = outlookbar.addtitle('系统设置', '系统设置', 1)
                for (var i = 0; i < mainInfo.length; i++) {
                    if(mainInfo[i].CategoryID=="1")
                    {
                        outlookbar.additem(mainInfo[i].Name, t, mainInfo[i].Code);
                    }                    
                }
                t = outlookbar.addtitle('业务管理', '业务管理', 1)
                for (var i = 0; i < mainInfo.length; i++) {
                    if(mainInfo[i].CategoryID=="0")
                    {
                        if(mainInfo[i].Name!="查看参会人员")
                        {
                          outlookbar.additem(mainInfo[i].Name, t, mainInfo[i].Code);
                        }                       
                    }                    
                }
//                outlookbar.additem('退出系统', t, 'AdminLogin.aspx?type=logout')
                initinav('业务管理');
            }
            else {
            }
