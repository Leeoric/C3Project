...
function addMeetingUpload(msg,val,errCode) { 
    if(errCode=="1"){            
        ...
        if (jsonParams == false)
            return;            
        var parameterArray = new Array(JSON2.stringify(jsonParams),1);
        
        AjaxInvoke("CreateMeeting", parameterArray);
...
function AjaxInvoke(method, parameterArray) {
    ...
    Ajax.Request({ url: "AjaxSecureHandler.aspx", type: "POST", data: "data=" + encodeURIComponent(JSON2.stringify(dataParameters)), onComplete: renderCreateMeetingResult, onError: showErrorHandle });
}
