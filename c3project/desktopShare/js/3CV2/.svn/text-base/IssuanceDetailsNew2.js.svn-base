    ...
    if (result && result.Data) {
	 ...
	 var nowtime = new Date().format("yyyy-MM-dd hh:mm:ss")
        if (nowtime >= beginTime && nowtime <= endTime) {
            var href_live = $("href_live");
            href_live.style.display = "";
            href_live.onclick = (function () {
                return function () {
                    var url = "/3CWeb/3CV32/vlc.html?meetingid={0}".format(result.MeetingID)
