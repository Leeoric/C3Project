...
function pushMyContactsList(list) {
    ...
    try {
        ...
        //JavaChu新增im列表
        for (var i = 0; i < imuserlistobj.Members.length; i++) {
            num++;
        }
    } catch (err) {
        ...
        if (result.State == 0) {
            if (result.Data != null) {
                ...
                {    
                    ...
                    //默认执行以下双击WM临时聊天组的动作
                    loadContact(document.getElementById('group_'+num),'WM临时讨论组',num);
                    
                    for(var i=0;i<imUserList1.UserList.length;i++)
                    {
                    
                        chooseMyContact(imUserList1.UserList[i].UserID,'WM临时讨论组');
                    }
                }catch(err)
                {}
            }
...
function getWMGroup()
{
    try
    {
        //var imUserList="{ LoginUserId: 'UserIdA',GroupId: 'GroupId',Members: [{ MemberUserId: 'UserIdB', Phone: 'PhoneB', EMail: 'ExampleB@DomainB' }, { MemberUserId: 'UserIdC', Phone: 'PhoneC', EMail: 'ExampleC@DomainC' } ] }";
        var  temp ="{'Key':'WM临时讨论组','UserList':[";
        ...
        for(var i=0;i<imUserListObj.Members.length;i++)
        {
            ...
            var username =imUserListObj.Members[i].UserName;
            var userid =imUserListObj.Members[i].UserId;
            var email =imUserListObj.Members[i].EMail;
            var phone =imUserListObj.Members[i].Phone;
            var company =imUserListObj.Members[i].CompanyName;
            
            if(imUserListObj.Members.length-1>i)
            {
                temp+="{'UserName':'"+username+"','UserID':'"+userid+"','Company':'Wind资讯','Department':'','Email':'"+email+"','IMID':'"+userid+"','Memo':'','MobilePhone':'"+phone+"','Position':'','Telephone':'','UserFrom':'2','UserRank':0,'Type':0},";
            }
            else
            {
                temp+="{'UserName':'"+username+"','UserID':'"+userid+"','Company':'Wind资讯','Department':'','Email':'"+email+"','IMID':'"+userid+"','Memo':'','MobilePhone':'"+phone+"','Position':'','Telephone':'','UserFrom':'2','UserRank':0,'Type':0}";
...
function buildGroupList(myList, isBrand) {
    ...
    //JavaChu新增把WM的对象增加到联系人列表中
    content.append("<div id=\"group_"+temp+"\" onclick=\"loadContact(this,'WM临时讨论组','"+temp+"')\" loaded=\"0\" class=\"group\">WM临时讨论组</div><div id=\"contact_"+temp+"\" style=\"display:none\"></div></div>");
